...
public class FractalWave implements Shape, Drawable {
    ...
    public void draw(Graphics g) {
        ...
        drawTinyWave(g,main.getFrame().getWidth() / 2 + 300,  main.getFrame().getHeight()+ 10, -180, 11);
        drawTinyWave2(g, main.getFrame().getWidth() / 2 + 180, main.getFrame().getHeight()+10 , 0, -10, 12);
    }
    ...
    private void drawTinyWave(Graphics g, int x1, int y1, double angle, int depth) {
        ...
        g.drawLine(x1, y1, x2, y2);
        drawTinyWave(g, x2, y2, angle - 15, depth - 1);
        drawTinyWave(g, x2, y2, angle + 15, depth - 1);
    }
    private void drawTinyWave2(Graphics g, int centerX, int centerY, double angle, double rotationAngle, int depth) {
        ...
        int waveLengthX = depth * 10; // Componente X
        int waveLengthY = depth * 5;  // Componente Y
